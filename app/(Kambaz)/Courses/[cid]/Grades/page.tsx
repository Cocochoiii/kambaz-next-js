...
// The Grades screen. Canvas shows two screens here, so I do too.
// Faculty gets one row per student and one column per assignment, and can
// click a cell to change a score. A student only sees their own scores.
...
import { updateGrade, releaseGrades } from "./reducer";
...
// The titles look like "A1: CSS & Layout". Canvas only shows the short
// name here, so I keep the part before the colon.
...
export default function Grades() {
  ...
  // The cell I am editing, and its score. The score lives here, so the
  // dialog shows the score of the cell I just clicked.
  const [cell, setCell] = useState<any>(null);
  const [score, setScore] = useState("");

  const courseAssignments = assignments.filter((a: any) => a.course === cid);

  // The students of this course. A TA or a teacher has no grades.
  const students = db.users.filter(
    (user: any) =>
      user.role === "STUDENT" &&
      enrollments.some(
        (e: any) => e.user === user._id && e.course === cid
      )
  );
  ...
  if (isFaculty) {
    return (
      <div id="wd-grades">
        ...
        {/* responsive gives a scrollbar when the table is too wide */}
  ...
  const weights = db.gradeCategories.find((c: any) => c.course === cid);
                    ...
                    <td>
                      {hasScore ? (
                        <>
                          ...
                          {/* A bar made with a width and a background color */}
                          <div
                            className="mt-1"
